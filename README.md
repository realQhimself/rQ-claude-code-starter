...
```
powershell -ExecutionPolicy Bypass -Command "irm https://raw.githubusercontent.com/realQhimself/rQ-claude-code-starter/main/setup.ps1 | iex"
```
...
跟 Claude 说"帮我写一篇关于时间管理的文章大纲" — 如果它用中文回答，回答有结构、有深度，就说明配置生效了。
...
**你遇到的问题**：Claude 从文件里读到了密码或 API 密钥，然后贴在聊天记录里

**这条规则干了什么**：如果 Claude 从文件里读到了密码或密钥，不许在回复里重复展示，而是引导你自己填到安全的文件里

**如果没有这条**：Claude 可能在解释代码时把从文件里读到的密钥直接贴出来

---

**你遇到的问题**：Claude 很自信地告诉你一个信息，但其实是编的

**这条规则干了什么**：不确定的事必须说"我不确定"，然后提议帮你搜索
...
打开配置文件夹里的 `CLAUDE.md`，在最后面加你的规则就行。写英文效果最好，写中文也能用。
...
ren CLAUDE.md.backup.TIMESTAMP CLAUDE.md
ren settings.json.backup.TIMESTAMP settings.json
...
```powershell
cd $env:USERPROFILE\.claude
del CLAUDE.md, settings.json, settings.local.json
```
